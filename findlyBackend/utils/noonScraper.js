...
async function scrapeNoon(query, minPrice, maxPrice) {
  return await withRetry(async () => {
    ...
    // ✅ Noon's internal catalog API — no browser needed
    const response = await axios.get(
      "https://www.noon.com/_svc/catalog/api/v3/search",
      {
        params: {
          q: query,
          lang: "en",
    ...
    console.log(chalk.gray("→ Noon API status:"), response.status);

    const data = response.data;
    console.log(chalk.gray("→ Noon API keys:"), Object.keys(data || {}));
